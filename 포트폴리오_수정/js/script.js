$(document).ready(function() {
  ...
  $('#ipod .btn_ipod').on('click', function(){
    ...
    $ipodLi.children().dblclick(function(){
      ...
      $('html, body').stop().animate({scrollTop: cntY[listNum]}, 900, 'easeOutElastic', a11y);
  ...
  $(window).on('scroll', function(){
    ...
    timerScroll = setTimeout(function(){
      $ipodLi.children().each(function(idx){
  ...
  $(window).scroll(function(){
    ...
    for (let s = 0; s < 6; s++){
      ...
      $('.bg_sulwhasoo p').eq(0).css({left: bgSulwhasooY[0] + "px"});
      $('.bg_sulwhasoo p').eq(1).css({right: bgSulwhasooY[1] + "px"});
      $('.bg_sulwhasoo p').eq(2).css({left: bgSulwhasooY[2] + "px"});
      $('.bg_sulwhasoo p').eq(3).css({right: bgSulwhasooY[3] + "px"});
      $('.bg_sulwhasoo p').eq(4).css({left: bgSulwhasooY[4] + "px"});
      $('.bg_sulwhasoo p').eq(5).css({right: bgSulwhasooY[5] + "px"});
    }
  });
...
$(window).on('scroll', function () {
  ...
  $('#cnt3').find('.sticky_btn button').on('click', function(){
    ...
    if($(this).is('.normal')){
      ...
      $pageLi.eq(tgNum).addClass('on').siblings().removeClass('on');
      $cntWrap.stop().animate({marginLeft: -tgNum * winWid}, 500, a11y);
      });
      // prev, next 버튼
      ...
      $pageLi.children().on('click', function () {
        ...
        tgNum = $(this).parent().index();
        $(this).parent().addClass('on').siblings().removeClass('on');
       $('html, body').stop().animate({scrollTop: $cnt3CntY[tgNum]}, 700, a11y);
      });
